...
/*
 * Bu dosya quiz sistemimizin tüm işleyişini barındırıldığı dosya olacak.
...
if(isset($_POST['question_id']) AND isset($_POST['answer'])){
    ...
    $_SESSION['answered_questions'][] = $answeredQuestionId; //sorunun idsini cevaplanan sorular dizisine atadık
    $isCorrect = $connection->query("SELECT id FROM questions WHERE id= $answeredQuestionId AND answer_id = $answerId")->rowCount();
    //yanıt doğruysa sessiondaki doğru yanıt sayısını 1 arttıralım.
    if($isCorrect == 1){
...
//implode kendisine gelen dizinin elemanları arasına virgül koyarak stringe dönüştürür.
        ...
        <div class="col-md-6 col-md-offset-3">
            <form method="post" class="form">
                ...
                <?php foreach($connection->query("SELECT * FROM question_options WHERE question_id = $questionId ORDER BY RAND()") as $option): ?>
                    ...
                    <label for="opt<?=$option['id']?>"><?=$option['body']?></label>
                <?php endforeach; ?>
